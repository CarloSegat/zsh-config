...
Before spawning investigators, list the MCP servers available in the Claude Code session. MCP tools are named `mcp__<server>__<tool>` (deferred ones load through ToolSearch); `claude mcp list` prints the configured servers.
...
- **Do not use a tool-restricted agent such as `qwen-worker`.** It has no MCP access, which disables MCP-backed investigators entirely. Investigators still shouldn't write anything.
...
- The synthesizer's quality check spot-verifies citations, which can require MCP access. A tool-restricted agent such as `qwen-worker` has no MCPs and defeats that.
